...
void Appearance::createMenus(){
    ...
    connect(saveToTexAct, SIGNAL(triggered()), this, SLOT(texFileSave()));
...
void Appearance::createTools(){
    ...
    setAcceptingState = new QPushButton(tr("&Accepting State"),this);
    setAcceptingState->setCheckable(false);
    setAcceptingState->setShortcut(tr("CTRL+D"));
    setAcceptingState->setChecked(false);
    connect(setAcceptingState, SIGNAL(clicked()), scribbleArea, SLOT(clickedAcceptingStateButton()));
    ...
    toolbar->addWidget(setAcceptingState);
...
bool Appearance::texFileSave(){
    QString initialPath = QDir::currentPath() + "/tex";

    QString saveTexFileName = QFileDialog::getSaveFileName(this,  tr("Save as"), initialPath, tr("TEX(*.tex)"));
    int pos = saveTexFileName.lastIndexOf('.');
    if (pos >= 0){
        return Tex->convertToTex(saveTexFileName);
    }
    else{
        return false;
    }
