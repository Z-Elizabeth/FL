...
void circle::drawNewCircle(const QPoint &endPoint) {
    int radius = 25;
    std::pair<QPoint, std::pair<int, bool>> oneMore(endPoint, {radius, false});
...
std::pair<QPoint, int> circle::checkNearCircle(const QPoint &point) {
    std::pair<QPoint, int> result;
    result.first = point;
    result.second = 0;
    for (auto elem : listCircles) {
        if ((point.x() >= elem.first.x() - elem.second.first) && (point.x() <= elem.first.x() + elem.second.first)
             && (point.y() >= elem.first.y() - elem.second.first) && (point.y() <= elem.first.y() + elem.second.first)) {
            result = {elem.first, elem.second.first};
        }
    }
    return result;
}
...
void circle::changeSizeCircle(const QPoint &endPoint){
    ...
    int radius = a*a + b*b;
    radius = sqrt(radius);
    listCircles[numCurCircle].second.first = radius;
    QPoint point;
    point.setX(listCircles[numCurCircle].first.x()-listCircles[numCurCircle].second.first);
    point.setY(listCircles[numCurCircle].first.y()-listCircles[numCurCircle].second.first);
    listRect[0].first = point;
    listRect[0].second = 2*listCircles[numCurCircle].second.first;
}
...
std::vector<int> circle::ifEmptyCircle(const QPoint &endPoint){
    ...
    for(int i=0; i<size; i++) {
        ...
        int res = a*a + b*b - listCircles[i].second.first*listCircles[i].second.first;
        numCurCircle= i;
        if(res < 0) return {1,listCircles[i].first.x(),listCircles[i].first.y(),listCircles[i].second.first};
    }
    return parametrs;
}
