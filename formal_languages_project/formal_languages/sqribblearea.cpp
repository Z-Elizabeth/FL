...
void ScribbleArea::clickedAcceptingStateButton()
{
    if (Circle.ifMarkCircle()) {
        if (Circle.listCircles[Circle.numCurCircle].second.second) {
            Circle.listCircles[Circle.numCurCircle].second.second = false;
        }
        else {
            Circle.listCircles[Circle.numCurCircle].second.second = true;
        }
        modified = true;
    }
    redraw();
...
void ScribbleArea::redraw(){
    ...
    for(int i=0; i<size; i++) {
        painter.drawEllipse(Circle.listCircles[i].first, Circle.listCircles[i].second.first, Circle.listCircles[i].second.first);
        if (Circle.listCircles[i].second.second) {
            painter.drawEllipse(Circle.listCircles[i].first, Circle.listCircles[i].second.first - 5, Circle.listCircles[i].second.first - 5);
        }
    }
    ...
    if (file.open(QIODevice::WriteOnly))
    {
        ...
        for(int i=0; i<size; i++){
           ...
           if (Arrow.listArrows[i].first.x() != Arrow.listArrows[i].second.x() or Arrow.listArrows[i].first.y() != Arrow.listArrows[i].second.y()) {
                QString str = "0|" + Text.listArrowTexts[i].second + "|"
                    + QString::number(Text.listArrowTexts[i].first.x()) + "|"
                    + QString::number(Text.listArrowTexts[i].first.y()) + "|"
                    + QString::number(Arrow.listArrows[i].first.x()) + "|"
        ...
        for(int i=0;i<size; i++){
            ...
            QString str = "1|" + Text.listCircleTexts[i].second + "|" + QString::number(Circle.listCircles[i].first.x())+"|"
                    ...
                    + QString::number(Circle.listCircles[i].second.first) + "\n";
            file.write(str.toUtf8());

            if (Circle.listCircles[i].second.second) {
                QString str = "1|" + Text.listCircleTexts[i].second + "|" + QString::number(Circle.listCircles[i].first.x())+"|"
                        + QString::number(Circle.listCircles[i].first.y()) + "|"
                        + QString::number(Circle.listCircles[i].second.first - 5) + "\n";
                file.write(str.toUtf8());
            }
        }
